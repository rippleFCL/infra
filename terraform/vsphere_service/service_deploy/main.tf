...
module "docker-gpu" {
  ...
  vm_host_id      = data.vsphere_host.esxi2.id
  vm_template     = "deb-x12-template-prod"
  vm_domain = "int.ripplefcl.com"
  vm_folder = vsphere_folder.vm_folder.path
  vm_tags   = [vsphere_tag.docker-gpu-host.id, vsphere_tag.docker-host.id, vsphere_tag.debian.id]
  ...
  network_spec = [
    {
      network_id = data.vsphere_network.compute_cu_vm_vlan.id
    },
    {
      network_id = data.vsphere_network.compute_cu_vm_storage_vlan.id
    },
  ]
  spec = {
    cpu       = 6
...
module "network-services" {
  ...
  vsphere_cluster = "compute-cluster"
  is_ha           = false
  vm_name         = "network-services-${format("%02d", count.index + 1)}-prod"
  vm_hostname     = "network-services-${format("%02d", count.index + 1)}"
  vm_template     = "deb-x12-template-prod"
  vm_domain = "int.ripplefcl.com"
  vm_folder = vsphere_folder.vm_folder.path
